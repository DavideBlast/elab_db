    ...
    \definecolor{LightCyan}{rgb}{0.88,1,1}
        ...
        \textbf{messaggi}(\underline{email}, \underline{idAnnuncio}, \underline{timestamp}, testo, mittente) \\
            ...
            \texttt{
                ...
                FROM (Immobili I JOIN Annunci\_Utente A ON (A.idImmobile = I.idImmobile)) J JOIN Zone Z ON (J.idZona = I.idZona AND Z.idCittà = ? AND StatoAnnuncio = 'Attivo') \\
            ...
            \texttt{
                \textcolor{blue}{SELECT} * \\
                FROM (Immobili I JOIN Annunci\_Utente A ON (A.idImmobile = I.idImmobile)) J JOIN Zone Z ON (J.idZona = I.idZona AND Z.idCittà = ? AND StatoAnnuncio = 'Attivo' AND tipoAnnuncioUtente = ?) \\
                \textcolor{blue}{ORDER BY} J.DataCreazione  \textcolor{blue}{DESC} \\
            }
            aste: \\
            \texttt{
                \textcolor{blue}{SELECT} * \\
                FROM (Immobili I JOIN Aste A ON (A.idImmobile = I.idImmobile)) J JOIN Zone Z ON (J.idZona = I.idZona AND Z.idCittà = ? AND A.dataFine > NOW()) \\
                \textcolor{blue}{ORDER BY} J.DataCreazione  \textcolor{blue}{DESC} \\
            }
            ...
            SELECT * \\
            FROM immobili I JOIN annunci\_utente A ON (I.idImmobile = A.idImmobile AND A.statoAnnuncio = “Attivo” AND I.tipoImmobile = ?) \\
            WHERE EXISTS (SELECT * \\
                            FROM zone Z, città C \\
                            WHERE C.idCittà = ? \\
                            AND Z.idCittà = C.idCittà \\
            ...
            \texttt{
                \textcolor{blue}{SELECT} * \\
                FROM immobili I JOIN annunci\_utente A ON (I.idImmobile = A.idImmobile AND A.statoAnnuncio = “Attivo” AND I.idZona = ?)
            ...
            \textbf{OP 8: Ordinare gli annunci di affitto per costo mensile} \\
            SELECT * \\
            FROM Immobili I JOIN annunci\_utente A ON (I.idImmobile = A.idImmobile AND A.statoAnnuncio = 'Attivo' AND A.tipoAnnuncioUtente = 'Affitto') \\
            WHERE EXISTS (SELECT * \\
                            FROM zone Z, città C \\
                            WHERE C.idCittà = ? \\
                            AND Z.idCittà = C.idCittà \\
                            AND Z.idZona = I.idZona) \\
            ORDER BY A.costoMensiile DESC \\
            
            \noindent
            \textbf{OP 10: Mostrare aste attive in una città} \\
            \texttt{
                \textcolor{blue}{SELECT} * \\
                FROM (Immobili I JOIN Aste A ON (A.idImmobile = I.idImmobile)) J JOIN Zone Z ON (J.idZona = I.idZona AND Z.idCittà = ? AND J.dataFine > NOW()) \\
                \textcolor{blue}{ORDER BY} J.DataCreazione  \textcolor{blue}{DESC} \\
            }
            ...
            \textbf{OP 11: Ordinare le aste per prezzo attuale crescente} \\
            \texttt{
                SELECT idAnnuncio, IF((SELECT COUNT(*) \\
                                        FROM rialzi R \\
                                        WHERE R.idAnnuncio = idAnnuncio) == 0, prezzoMinimo, (DECLARE prezzoMassimo float; \\
                                                                                              SELECT TOP(1) prezzoAttuale \\
                                                                                              INTO prezzoMassimo \\
                                                                                              FROM rialzi R \\
                                                                                              WHERE R.idAnnuncio = idAnnuncio \\
                                                                                              ORDER BY prezzoAttuale DESC; \\
                                                                                              prezzoMassimo)) \\
                FROM aste \\
                WHERE dataFine > NOW() \\
            }
            ...
            \textbf{OP 12: Effettuare un rialzo all’interno di un’asta (controllando la sua validità)} \\
            \texttt{
                IF ((SELECT MAX(prezzoAttuale) \\
                FROM Rialzi \\
                WHERE idAnnuncio = ? ) <= ? - (SELECT rialzoMinimo \\
                                                FROM Aste \\
                                                WHERE idAnnuncio = ?), \\
                                                INSERT INTO Rialzi(prezzoAttuale, e-mail, idAnnuncio, dataRialzo) \\
                                                VALUES(?, ?, ?, ?), NULL ) \\
            }       
            ...
            \textbf{OP 17: Comparare il prezzo di un immobile al mq con quello degli immobili nella stessa città} \\
            \texttt{
                SET @costoMedioMqCitta = (SELECT SUM(costoMedioMq*numeroImmobili) \\
                                            FROM Zone \\
                                            WHERE idCittà = ?) / SELECT SUM(numeroImmobili) \\
                                                                    FROM Zone  \\
                                                                    WHERE idCittà = ? \\
                \textcolor{blue}{SELECT} ((prezzo/metriQuadri)-@costoMedioMqCitta) * 100/@costoMedioMqCitta \\
                \textcolor{blue}{FROM} (Immobili I \textcolor{blue}{JOIN} Annunci\_Utente A  \\
                \textcolor{blue}{ON} (A.idImmobile = I.idImmobile)) J \textcolor{blue}{JOIN} Zone Z \\
                \textcolor{blue}{ON} (J.idZona = I.idZona) JOIN Città C \\
                ON (C.idCittà = Z.idCittà) \\
                \textcolor{blue}{WHERE} idImmobile = ? \textcolor{blue}{AND} statoAnnuncio = 'Attivo' \\
            }
            \noindent
            \textbf{OP 18: Comparare il prezzo medio al mq di una zona con quello della città} \\
            \texttt{
                ...
                \textcolor{blue}{WHERE} idCittà = ? AS percZonaCitta\\
            ...
            \noindent
            \textbf{OP 21: Classificare città in base all’evoluzione in una o più categorie rispetto all’anno precedente} \\
            SELECT idCittà, anno, punteggioCittà
            
            SELECT punteggioAmbiente+punteggioTrasporto+punteggioEconomia+punteggioSanità+punteggioIstruzione AS punteggioCittà, anno \\
            FROM città\_anni \\
            WHERE idCittà = ? AND anno = ? \\
            \noindent
            \textbf{OP 22: Ordinare le città in base a valori specifici di una o più categorie} \\
            \texttt{
            ...
            FROM (stati S JOIN città C ON (S.nome = C.nomeStato)) J \\
                    JOIN città\_anni A ON (A.idCittà = J.idCittà AND anno = ?) \\
            WHERE J.nomeStato = ? \\

            \noindent
            \textbf{OP 24: Aggiornamento annuale dei punteggi di una città (dati i campi delle categorie)} \\
            newHashAmbiente = CAST(PM2.5media*10 AS int)*1001 + CAST(percentualeSpazioVerdeUrbano*10 AS int) \\
            IF ((SELECT COUNT(hashAmbiente) \\
                FROM Ambiente \\
                WHERE hashAmbiente = newHashAmbiente) > 0,  \\
            NULL,  \\
            INSERT INTO Ambiente(hashAmbiente, PM2.5media, percentualeSpazioVerdeUrbano) \\
            VALUES (newHashAmbiente, ?, ?)) \\
 \\
            INSERT INTO Città\_Anni(punteggioAmbiente, newHashAmbiente) \\
