...
\begin{document}
    ...
    \definecolor{LightCyan}{rgb}{0.88,1,1}
        ...
        stati(\underline{nome}, superficie, popolazione) \\
        città(\underline{idCittà}, nome, popolazione, superficie, regione*, nomeStato) \\
            FK: nomeStato REFERENCES stati
        città\_anni(\underline{idCittà}: città, \underline{anno}: anni, punteggioAmbiente, punteggioTrasporto, punteggioEconomia, punteggioSanità, punteggioIstruzione) \\
        anni(\underline{anno}) \\
        ambiente(\underline{idCittà}: città, \underline{anno}: anni,  PM2.5media, percentualeSpazioVerdeUrbano) \\
        trasporto(\underline{idCittà}: città, \underline{anno}: anni, percorrenzaMediaPendolare, autoProCapite) \\
        economia(\underline{idCittà}: città, \underline{anno}: anni, PILProCapite, stipendioMedio, tassoDisoccupazione) \\
        sanità(\underline{idCittà}: città, \underline{anno}: anni, aspettativaVita, ospedaliProCapite) \\
        istruzione(\underline{idCittà}: città, \underline{anno}: anni, percentualeLaureati, percentualeDiplomati, numeroUniversità) \\
        zone(\underline{idZona}, \underline{idCittà}: città, nome, costoMedioMq, numeroImmobili) \\
        immobili(\underline{idImmobile}, metriQuadri, classeEnergetica*, annoCostruzione*, via, numeroCivico, numeroInterno*, piano*, numeroStanze*, numeroConquilini*, tipoImmobile) \\
        annunci\_utente(\underline{idAnnuncio}, \underline{idImmobile}: immobili, \underline{email}: utenti, statoAnnuncio, dataCreazione) \\
        aste(\underline{idAnnuncio}, \underline{idImmobile}: immobili, \underline{email}: giudici\_esecuzione, prezzoMinimo, rialzoMinimo, depositoCauzionale, dataFine, statoAnnuncio, dataCreazione) \\
        rialzi(\underline{prezzoAttuale}, \underline{idAnnuncio}: aste, \underline{email}: utenti, prezzoAttuale, dataRialzo) \\
        giudici\_esecuzione(\underline{email}, telefono*, nome) \\
        utenti(\underline{email}, telefono*, nome) \\
        messaggi(\underline{email}: utenti, \underline{idAnnuncio}: annunci\_utente, \underline{timestamp}, testo, mittente) \\
