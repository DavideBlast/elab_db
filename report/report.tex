...
\begin{document}
    ...
    \definecolor{LightCyan}{rgb}{0.88,1,1}
        ...
        \textbf{messaggi}(\underline{email}, \underline{idAnnuncio}, \underline{timestamp}, testo, mittente) \\
            ...
            SELECT * \\
            FROM Annunci\_Utente, Aste \\
            WHERE idCittà = ? AND StatoAnnuncio = 'Attivo' AND (Annunci\_Utente.TipoAnnuncioUtente = ? OR Aste.PrezzoMinimo IS NOT NULL) \\
            ORDER BY DataCreazione DESC \\
            ...
            SELECT * \\
            FROM Annunci\_Utente, Aste \\
            WHERE idZona = ? AND StatoAnnuncio = 'Attivo' \\
            ORDER BY DataCreazione DESC \\
            ...
            SELECT * \\
            FROM  Aste \\
            WHERE idCittà = ? AND StatoAnnuncio = 'Attivo' \\
            ORDER BY DataCreazione DESC \\
            ...
            \textbf{OP 21: Classificare città in base all’evoluzione in una o più categorie rispetto all’anno precedente} \\
             \\
            \textbf{OP 22: Ordinare le città in base a valori specifici di una o più categorie} \\
            SELECT A.percentualeSpazioVerdeUrbano, A.idCittà, C.nome, C.anno, C.idCittà \\
            FROM Ambiente A, Città\_Anni C \\
            WHERE A.idCittà = C.idCittà  \\
            AND C.anno = ? \\
